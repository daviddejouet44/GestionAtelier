...
app.MapPut("/api/fabrication", async (HttpContext ctx) =>
{
    try
    {
        ...
        var sheet = new FabricationSheet
        {
            ...
            MoteurImpression = input.MoteurImpression ?? old?.MoteurImpression,
            Machine          = input.MoteurImpression ?? input.Machine ?? old?.Machine,
            Operateur        = old?.Operateur,
            Quantite         = input.Quantite         ?? old?.Quantite,
            TypeTravail      = input.TypeTravail      ?? old?.TypeTravail,
            Format           = input.Format           ?? old?.Format,
            Papier           = input.Papier           ?? old?.Papier,
            RectoVerso       = input.RectoVerso       ?? old?.RectoVerso,
            Encres           = input.Encres           ?? old?.Encres,
            Client           = input.Client           ?? old?.Client,
            NumeroAffaire    = input.NumeroAffaire    ?? old?.NumeroAffaire,
            NumeroDossier    = input.NumeroDossier    ?? old?.NumeroDossier,
